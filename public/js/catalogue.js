    ...
    grid.innerHTML = formations.map(f => `
        <div class="formation-card" onclick="window.location.href='/formation/${f.id}'">
            <div class="formation-image">
                <img src="${f.image || 'https://via.placeholder.com/300x180/6366f1/ffffff?text=' + encodeURIComponent(f.title)}" alt="${f.title}">
...
document.getElementById('searchInput')?.addEventListener('input', (e) => {
    ...
    const filtered = allFormations.filter(f => 
        ...
        (f.short_description && f.short_description.toLowerCase().includes(query))
    );
    displayFormations(filtered);
});
